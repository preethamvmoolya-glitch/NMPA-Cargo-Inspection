  ...
  window.fetch = async function (url, options = {}) {
    ...
    try {
      ...
      if (pathname.endsWith("/api/public-metrics") && method === "GET") {
        ...
        const manifestsProcessed = inspections.filter(i => i.status === "Approved" || i.status === "Port Clearance Granted").length;
        const activeHolds = inspections.filter(i => i.status === "Clearance Denied - Detained for Physical Audit" || i.status === "Rejected").length;
        const totalGrievances = complaints.length;
        const escalatedGrievances = complaints.filter(c => c.sla_status === "SLA Breached" || c.escalated_to_chairman === true || c.escalatedToChairman === true).length;

...
const printCertificate = (record, qrToken, qrDataUrl, language, t) => {
  ...
  const effectiveToken = qrToken || record.qrToken || record.qr_token || (isDetained 
    ? `NMPA-QDO-${record.id || '101'}-${Math.floor(100000 + Math.random() * 900000)}` 
    : `NMPA-PCC-${record.id || '101'}-${Math.floor(100000 + Math.random() * 900000)}`);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (status === 'Port Clearance Granted' || status === 'Approved') {
          ...
          const qrToken = data.qrToken || activeDossier.qrToken || activeDossier.qr_token || `NMPA-QDO-${activeDossier.id}-${Math.floor(100000 + Math.random() * 900000)}`;
          const verifyUrl = `${window.location.origin}${import.meta.env.BASE_URL || '/'}verify-clearance?token=${qrToken}`;

          Modal.error({
            title: language === 'en' ? 'QUARANTINE & DETAINMENT ORDER (QDO) ISSUED' : 'क्वारंताइन और निरोध आदेश (QDO) जारी किया गया',
            okText: t('done'),
            content: (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px', marginTop: '10px' }}>
                <Text type="danger" strong style={{ fontSize: '1.05rem', textAlign: 'center' }}>
                  {language === 'en' 
                    ? `Vessel ${activeDossier.vesselName || activeDossier.manifestId} has been placed under active Enforcement Hold.`
                    : `पोत ${activeDossier.vesselName || activeDossier.manifestId} को निरोध के तहत रखा गया है।`}
                </Text>
                <div style={{ padding: '10px', background: '#fff', border: '1px solid #ff4d4f', borderRadius: '4px' }}>
                  <QRCode value={verifyUrl} size={160} color="#b71c1c" />
                </div>
                <Text style={{ fontFamily: 'monospace', fontSize: '0.85rem', color: '#b71c1c', fontWeight: 'bold' }}>
                  QDO Code: {qrToken}
                </Text>
                <Button 
                  type="primary" 
                ...
                >
                  {language === 'en' ? 'Print Detainment Order (QDO)' : 'निरोध आदेश (QDO) प्रिंट करें'}
                </Button>
              </div>
            ),
          });

          // Auto-trigger QDO certificate print after short render delay
          setTimeout(() => {
            const canvas = document.querySelector('.ant-modal-body canvas');
            const qrDataUrl = canvas ? canvas.toDataURL() : '';
            printCertificate(activeDossier, qrToken, qrDataUrl, language, t);
  ...
  const handleQuickAction = async (id, targetStatus) => {
    try {
      ...
      if (response.ok) {
        ...
        } else if (targetStatus === 'Clearance Denied - Detained for Physical Audit' || targetStatus === 'Rejected') {
          const record = inspectedCargoList.find(item => item.id === id) || {
            manifestId: `BL-NMPA-2026-${id}`,
            id: id,
            status: 'Clearance Denied - Detained for Physical Audit'
          };
          const qrToken = data.qrToken || record.qrToken || record.qr_token || `NMPA-QDO-${id}-${Math.floor(100000 + Math.random() * 900000)}`;
          const verifyUrl = `${window.location.origin}${import.meta.env.BASE_URL || '/'}verify-clearance?token=${qrToken}`;
          ...
          Modal.error({
            ...
            content: (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px', marginTop: '10px' }}>
                <Text type="danger" strong style={{ fontSize: '1.05rem', textAlign: 'center' }}>
                  {language === 'en' 
                    ? `Manifest ID ${id} detained for physical custom audit.` 
                    : `मैनिफेस्ट आईडी ${id} को भौतिक सीमा शुल्क ऑडिट के लिए रोक लिया गया।`}
                </Text>
                <div style={{ padding: '10px', background: '#fff', border: '1px solid #ff4d4f', borderRadius: '4px' }}>
                  <QRCode value={verifyUrl} size={160} color="#b71c1c" />
                </div>

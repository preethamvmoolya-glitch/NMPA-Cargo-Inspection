...
# Create the full python generator script for NMPA_Project_Report_Final.docx
...
def add_styled_paragraph(doc, text, indent=False, space_after=6):
    clean_txt = clean_text(text)
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    p.paragraph_format.space_after = Pt(space_after)
    p.paragraph_format.line_spacing = 1.5 # Standard academic 1.5 Line Spacing
    if indent:
...
def add_custom_table(doc, col_widths, headers, rows_data):
...
print("Generator helpers compiled successfully.")
'''

with open('generate_report_docx.py', 'w', encoding='utf-8') as f:
    f.write(script_content)

print("Updated generate_report_docx.py header base.")
